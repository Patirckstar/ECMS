...
export function batchOnShelf(ids: number[]) {
  return request.post<any, ApiResult<null>>(`${BASE_URL}/batch-on-shelf`, { ids })
}
...
export function batchOffShelf(ids: number[]) {
  return request.post<any, ApiResult<null>>(`${BASE_URL}/batch-off-shelf`, { ids })
}
 ...
 * 批量调整库存
 */
export function batchAdjustStock(spuId: number, items: { skuId: number; qty: number }[]) {
  return request.post<any, ApiResult<null>>(`${BASE_URL}/${spuId}/skus/batch-adjust-stock`, { items })
}
...
/**
 * 获取SPU的SKU组合（用于规格配置页自动生成）
 */
export function generateSkuCombinations(spuId: number, specIds: number[]) {
  return request.post<any, ApiResult<SkuItem[]>>(`${BASE_URL}/${spuId}/skus/generate`, { specIds })
}
