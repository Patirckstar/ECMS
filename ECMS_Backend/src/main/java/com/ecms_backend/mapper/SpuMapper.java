...
public interface SpuMapper {
	...
	@Select("<script>" +
			...
			"(SELECT MIN(sku.sale_price) FROM sku WHERE sku.spu_id = spu.id AND sku.status = 1) as minPrice, " +
			"(SELECT COALESCE(SUM(sku.stock),0) FROM sku WHERE sku.spu_id = spu.id AND sku.status = 1) as totalStock, "
			+
			"0 as totalSales " +
			"FROM spu " +
			...
			"AND EXISTS (SELECT 1 FROM sku WHERE sku.spu_id = spu.id AND sku.status = 1 GROUP BY sku.spu_id HAVING MIN(sku.sale_price) &gt;= #{params.priceMin}) "
			+
			"</if>" +
			"<if test='params.priceMax != null'>" +
			"AND EXISTS (SELECT 1 FROM sku WHERE sku.spu_id = spu.id AND sku.status = 1 GROUP BY sku.spu_id HAVING MIN(sku.sale_price) &lt;= #{params.priceMax}) "
			+
			"</if>" +
